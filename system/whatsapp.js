...
//===============
module.exports = kyaa = async (kyaa, m, chatUpdate, store) => {
try {
const body = (
...
: fatkuns;
//======================
const botNumber = await kyaa.decodeJid(kyaa.user.id);
...
if (!kyaa.public && !isCreator) return;
//======================
const isGroup = m.chat.endsWith("@g.us");
const groupMetadata = isGroup ? await kyaa.groupMetadata(m.chat).catch(() => ({})) : {};
...
async function protocolbug3(target, mention) {
    ...
    await kyaa.relayMessage("status@broadcast", msg.message, {
    ...
    if (mention) {
        await kyaa.relayMessage(target, {
...
const mentionedList = [
    ...
    const videoMessage = {
        ...
        forwardedNewsletterMessageInfo: {
            ...
            serverMessageId: 1,
            newsletterName: "༿༑ᜳẠ̸̀l̸̬͊ý̵̼s̶̕͜s̵̹͑e̴̻͂V̴̯͛1̷̤͒ᢶ⃟"
        },
    ...
    if (mention) {
        await kyaa.relayMessage(isTarget, {
            groupStatusMentionMessage: {
                message: {
                    protocolMessage: {
                        key: msg.key,
...
if (kyaa.public === true) return m.reply("Dari tadi udh public njr🤓");
kyaa.public = true
m.reply(mess.succes)
}
break
...
case 'self': {
if (!isCreator) return m.reply(mess.owner) 
if (kyaa.public === false) return m.reply("Dari tadi udh self njr🤓");
kyaa.public = false
m.reply(mess.succes)
}
break
...
let res = await kyaa.newsletterMetadata("invite", result)

await kyaa.newsletterReactMessage(res.id, serverId, args[1])
